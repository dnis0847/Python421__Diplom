...
async function loadCourses() {
  try {
    ...
    const response = await fetch(
      `/courses/load-courses/?page=${currentPage}${categoryParam}`
      //   `/load-courses/?page=${currentPage}${categoryParam}&level=${level}&price=${price}&teacher=${teacher}&view=${view}&sort=${sort}`
    )
    if (!response.ok) {
...
document.getElementById('apply-filters').addEventListener('click', loadCourses)
document.getElementById('reset-filters').addEventListener('click', () => {
  document.getElementById('level-select').value = 'beginner'
  document.getElementById('price-select').value = 'paid'
  document.getElementById('teacher-select').value = 'paid'
